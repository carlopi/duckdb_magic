...
struct MagicFunctionLocalState : public FunctionLocalState {
    ...
    if (magic_load(magic_cookie, "/Users/carlo/duckdblabs/extension-template/build/release/vcpkg_installed/arm64-osx/share/libmagic/misc/magic.mgc") != 0) {
...
inline unique_ptr<FunctionLocalState> MagicFunctionLocalStateFun (ExpressionState &state, const BoundFunctionExpression & expr, FunctionData *bind_data) {
...
auto &fs = FileSystem::GetFileSystem(state.GetContext());	
	...
	magic_full = magic_buffer(localState.magic_cookie, buffer, bytes_read);	
	} else {
